...
class Data():
    ...
    def __init__(self,
        ...
        self.datafiles = []
        self.labels = pd.read_csv(os.path.join(dataroot_S2,
                                               'ben-ge-s_esaworldcover.csv'),
                                  index_col="patch_id")
    ...
    def __getitem__(self, idx):
        ...
        label_fractions2 = label_fractions.loc[label_fractions >= self.labels_threshold] # apply threshold
        labels_ranked = label_fractions2.index[np.argsort(label_fractions2.values)[::-1]][:min(len(label_fractions2), self.labels_n)].values
        ...
        sample = {'idx': idx,
                  ...
                  'tile': os.path.split(
                      self.datafiles[idx])[-1].replace('_all_bands.npy', ''),
                  'data_S2': data,
    ...
    def display(self, idx):
        ...
        ax.imshow(1.3*(imgdata-np.min(imgdata, axis=(0, 1)))/
                   (np.max(imgdata, axis=(0, 1))-
                    np.min(imgdata, axis=(0, 1)))+0.1)
